...
modeType = 0  # Hier kannst du die Auswahl des Modus ändern
selection = -1  # Ist eine Auswahl getroffen?
counter = 0  # Zähler, um nicht direkt auszuwählen, sondern erst nach einer gewissen Zeit
selectionSpeed = 7  # Multiplikator, um den Auswahlkreis schneller zu schließen
...
while True:
    ...
    hands, img = detector.findHands(img)  # mit Zeichnung der Hand
    # hands = detector.findHands(img, draw=False)  # ohne Zeichnung der Hand
    ...
    for i in range(3):
        if selectionList[i] != -1:
            imgBackground[636:636+65, 133+207*i:133+207*i+65] = listImgIcons[2*i+selectionList[i]-1]
